...
SESSION_FILE="/tmp/.hyperdl_session"
NOW_TS=$(date +%s)
SHOW_ART=1

if [ -f "$SESSION_FILE" ]; then
  LAST_TS=$(cat "$SESSION_FILE" 2>/dev/null || echo 0)
  if [[ "$LAST_TS" =~ ^[0-9]+$ ]] && (( NOW_TS - LAST_TS < 20 )); then
    SHOW_ART=0
  fi
fi
echo "$NOW_TS" > "$SESSION_FILE" 2>/dev/null || true
